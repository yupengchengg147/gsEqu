...
if __name__ == "__main__":
    ...
    parser.add_argument("--obj", type=str, help="The path to the output directory that stores the relighting ground truth.")

    parser.add_argument("--result_file", type=str, default="results.json", help="The path to save the results in a JSON file.")
    args = parser.parse_args()

    light_name_list = ["corridor", "golf", "neon"]
    ...
    for light_name in light_name_list:
        ...
        for idx in trange(num_test):
            with torch.no_grad():
                ...
                image = Image.open(os.path.join(args.gt_dir, f"{args.obj}_{light_name}",f"{idx}.png"))
                # /is/cluster/fast/pyu/data/nero/relight_gt/bell_golf_blender/rgb
                ...
                # Calculate metrics
                psnr_avg += get_psnr(gt_img.to(prediction.dtype), prediction).mean().double()
                ssim_avg += get_ssim(gt_img.to(prediction.dtype), prediction).mean().double()
                lpips_avg += lpips_fn(gt_img.to(prediction.dtype), prediction).mean().double()
